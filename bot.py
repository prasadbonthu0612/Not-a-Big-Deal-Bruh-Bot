...
# Page-frame processing. The PNG is kept as a separate asset so the visual
# design can be changed without changing the bot architecture.
MEME_FRAME_PATH = os.getenv("MEME_FRAME_PATH", "meme_frame.png").strip()
FRAME_OUTPUT_WIDTH = 1080
FRAME_OUTPUT_HEIGHT = 1920
FRAME_VIDEO_X = 60
FRAME_VIDEO_Y = 360
FRAME_VIDEO_WIDTH = 960
FRAME_VIDEO_HEIGHT = 1100
FRAME_FFMPEG_PRESET = os.getenv("FRAME_FFMPEG_PRESET", "ultrafast").strip() or "ultrafast"
FRAME_FFMPEG_CRF = os.getenv("FRAME_FFMPEG_CRF", "20").strip() or "20"
FRAME_FFMPEG_THREADS = max(1, int(os.getenv("FRAME_FFMPEG_THREADS", "1")))
...
# =========================
# PAGE FRAME / VIDEO COMPOSITION
# =========================

def _resolve_meme_frame_path():
    """Resolve the configured page frame relative to the bot file when needed."""
    configured = Path(MEME_FRAME_PATH).expanduser()
    if configured.is_file():
        return configured

    module_relative = Path(__file__).resolve().parent / configured
    if module_relative.is_file():
        return module_relative

    raise FileNotFoundError(
        f"Page frame PNG was not found: {MEME_FRAME_PATH}. "
        "Place meme_frame.png beside bot.py or set MEME_FRAME_PATH."
    )


async def _apply_meme_frame(
    file_path,
    job_id,
    ...
    """
    Compose the downloaded Reel into the supplied 1080x1920 page frame.

    This is intentionally an asyncio-managed FFmpeg subprocess rather than
    subprocess.run(). That keeps the Telegram bot event loop responsive while
    FFmpeg is encoding and lets us stream FFmpeg's machine-readable progress.
    """
    if not shutil.which("ffmpeg"):
        raise RuntimeError(
            "FFmpeg executable was not found in PATH. "
            "The page-frame processor requires ffmpeg."
        )

    source = Path(file_path)
    if not source.is_file():
        raise FileNotFoundError(f"Downloaded video not found: {source}")

    frame = _resolve_meme_frame_path()

    output = source.with_name(
        f"{source.stem}_framed_{job_id}.mp4"
    )

    filter_complex = (
        f"[0:v]"
        f"scale={FRAME_VIDEO_WIDTH}:{FRAME_VIDEO_HEIGHT}:"
        "force_original_aspect_ratio=decrease,"
        "format=rgba,"
        f"pad={FRAME_VIDEO_WIDTH}:{FRAME_VIDEO_HEIGHT}:"
        f"(ow-iw)/2:(oh-ih)/2:color=black@0"
        "[clip];"
        f"[1:v]scale={FRAME_OUTPUT_WIDTH}:{FRAME_OUTPUT_HEIGHT}:"
        "flags=fast_bilinear[frame];"
        f"[frame][clip]overlay={FRAME_VIDEO_X}:{FRAME_VIDEO_Y}:"
    ...
    if duration <= 0:
        ...
        if duration <= 0:
            raise RuntimeError(
                "Unable to determine source video duration for frame progress. "
                + probe_stderr.decode(errors="replace")[-1000:]
            )
    ...
    try:
        while True:
            ...
            if key == "out_time_ms":
                try:
                    current_time = max(0.0, float(value) / 1_000_000.0)
                except ValueError:
                    continue
            elif key == "speed":
                speed = value or "N/A"
            ...
            now = time.monotonic()
            percent = min(99.9, max(0.0, current_time * 100.0 / duration))

            # Bot API progress is deliberately throttled. FFmpeg can emit
            # updates much faster than Telegram should be edited.
            if (
                progress_callback is not None
                and (now - last_report >= 3.0 or percent >= 99.9)
                ...
                if speed.endswith("x"):
                    try:
                        ...
                        if speed_value > 0:
                            eta = max(0.0, duration - current_time) / speed_value
                    except ValueError:
                        pass
                ...
                try:
                    ...
                    print(
                        f"⚠️ Page-frame progress callback failed for {job_id}: {exc}"
                    )
        ...
        if return_code != 0:
            raise RuntimeError(
                "FFmpeg page-frame processing failed"
                + (f": {stderr_text[-2500:]}" if stderr_text else ".")
            )

        if not output.is_file() or output.stat().st_size <= 0:
            raise RuntimeError(
                "FFmpeg reported success but did not create a valid framed video."
            )
        ...
        if progress_callback is not None:
            try:
                ...
                print(
                    f"⚠️ Final page-frame progress callback failed for {job_id}: {exc}"
                )
        ...
        try:
            ...
            raise RuntimeError(
                f"Framed video was created, but the original downloaded file "
                f"could not be replaced: {exc}"
            ) from exc
        ...
        try:
            ...
            try:
                ...
                pass
            raise RuntimeError(
                f"Could not install the framed video: {exc}"
            ) from exc
...
async def process_one_job(job):
    ...
    try:
        ...
        #
        # The hash is persisted before frame encoding. If Render restarts
        # during the frame stage, recovery may re-download the source, but
        # it must not waste time hashing the same job again when a valid
        # fingerprint is already persisted.
        persisted_job = await telegram_storage.get_job(job_id)
        media_hash = str(
            (persisted_job or {}).get("media_hash", "")
        ).strip().lower()

        if media_hash:
            await update_processing_status(
                job,
                "🔐 Media fingerprint already available",
                "Using the persisted SHA-256 fingerprint; skipping a second hash pass.",
                finished=True,
            )
        else:
        ...
        await update_processing_status(
            job,
            "🎬 Applying page frame",
            "Composing the Reel inside the 1080×1920 page template...",
        )

        async def frame_progress(
            percent,
            current_time,
            duration,
            speed,
            ...
            if finished:
                detail = (
                    "Page frame encoding complete.\n"
                    f"Processed: {_duration_text(duration)}\n"
                    f"Encoding speed: {speed}"
                )
            elif error:
                detail = f"Page frame encoding failed: {error}"
            else:
                detail = (
                    "Composing the Reel inside the 1080×1920 page template...\n\n"
                    f"Progress: [{_progress_bar(percent)}] {percent:5.1f}%\n"
                    f"Video processed: {_duration_text(current_time)} / {_duration_text(duration)}\n"
                    f"Encoding speed: {speed}\n"
        ...
        framed_file_path = await _apply_meme_frame(
            file_path,
            job_id,
            progress_callback=frame_progress,
            duration_hint=(download_result.get("info", {}) or {}).get("duration"),
        )
        file_path = framed_file_path
        file_size = os.path.getsize(file_path)

        print("\n==============================")
        print("🎬 PAGE FRAME APPLIED")
        print(f"Job: {job_id}")
        print(f"Frame: {MEME_FRAME_PATH}")
