...
FRAME_FFMPEG_PRESET = os.getenv("FRAME_FFMPEG_PRESET", "veryfast").strip() or "veryfast"
...
def _apply_meme_frame(file_path, job_id):
    """
    Compose the downloaded Reel into the supplied 1080x1920 page frame.

    The source video is fitted (not stretched) inside the blue middle area.
    Transparent padding lets the frame's own background remain visible around
    portrait videos. The frame itself supplies the page logo/name and bottom
    reaction artwork. Source audio is retained unchanged in content and
    re-encoded only as required for the final MP4.
    """
    ...
    try:
        ...
        try:
            ...
            pass
        raise RuntimeError(
            "FFmpeg page-frame processing timed out after 15 minutes."
        ) from exc
    ...
    try:
        ...
        # If replacement failed, leave the original source untouched if it
        # still exists; otherwise clean the temporary output.
        try:
            if output.exists():
                output.unlink()
        except OSError:
...
async def process_one_job(job):
    ...
    try:
        ...
        framed_file_path = await asyncio.to_thread(
            _apply_meme_frame,
            file_path,
            job_id,
        )
